...
private extension KotlinGrammar {
    static let keywords = ([
        ...
        "super", "this", "set", "true", "false", "null",
        "override", "where", "_", "default", "break",
        "constructor", "lateinit", "lazy", "operator",
        "while", "continue", "repeat", "is"
    ...
    struct PreprocessingRule: SyntaxRule {
        ...
        private let controlFlowTokens: Set<String> = []
        private let directiveTokens: Set<String> = []
    ...
    struct CallRule: SyntaxRule {
        ...
        private let controlFlowTokens = ["if", "&&", "||", "for", "when"]
    ...
    struct KeywordRule: SyntaxRule {
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            if segment.tokens.current == "operator" && segment.tokens.next == "fun" {
                return true
            }

            if segment.tokens.next == ":", segment.tokens.current != "null" {
            ...
            if let previousToken = segment.tokens.previous {
                ...
                if !segment.tokens.onSameLine.isEmpty, segment.tokens.current != "this" {
                    guard !previousToken.isAny(of: "val", "var") else {
                        return false
                    }
                }
                ...
                if !declarationKeywords.contains(segment.tokens.current) {
                    ...
                    if !segment.tokens.current.isAny(of: "this", "val", "var", "true", "false", "null") {
                        guard !previousToken.isAny(of: "(", ",", ">(") else {
                            return false
                        }
                    }

                    guard !segment.tokens.previous.isAny(of: "fun", "`") else {
                        return false
                    }
                }
            }
